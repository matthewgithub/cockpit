                 ...
                 gconstpointer data)
{
  ...
  const gchar *expect_knownhosts = NULL;
  const gchar *command;
  ...
  if (fixture->expect_key)
    expect_knownhosts = g_strdup_printf ("[127.0.0.1]:%d %s", tc->ssh_port, fixture->expect_key);
                   ...
                   gconstpointer data)
{
  ...
  knownhosts = g_strdup_printf ("[127.0.0.1]:%d %s", tc->ssh_port, MOCK_RSA_KEY);
