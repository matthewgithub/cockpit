...
get_knownhosts_line (ssh_session session)
{
  ...
  if (ssh_write_knownhost (session) != SSH_OK)
    {
      g_warning ("Couldn't write knownhosts file.");
      return NULL;
    }

  if (!g_file_get_contents (name, &line, &len, &error))
    {
      g_warning ("Couldn't read '%s': %s", name, error->message);
      g_clear_error (&error);
      return NULL;
    }
  ...
  if (data->expect_key)
  ...
  else
    {
      ...
      if (state == SSH_SERVER_KNOWN_OK)
        {
          g_debug ("VERIFY %s: verified host key", data->logname);
          ret = NULL; /* success */
          goto done;
        }
